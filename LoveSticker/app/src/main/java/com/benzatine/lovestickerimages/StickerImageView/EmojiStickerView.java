...
import com.benzatine.lovestickerimages.R;

import static com.benzatine.lovestickerimages.Edit_Image.EmojiLastSelectedView;
import static com.benzatine.lovestickerimages.Edit_Image.LastSelectedView;

/**
 * Created by ADMIN on 10-Apr-17.
 */

public abstract class EmojiStickerView extends FrameLayout {
    ...
    public EmojiStickerView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public EmojiStickerView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context) {
        ...
        this.iv_delete.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View view) {
                if (EmojiStickerView.this.getParent() != null) {
                    ViewGroup myCanvas = ((ViewGroup) EmojiStickerView.this.getParent());
                    myCanvas.removeView(EmojiStickerView.this);
                }
            }
        });
            ...
            if (view.getTag().equals("DraggableViewGroup")) {
                switch (event.getAction()) {
                    case MotionEvent.ACTION_DOWN:
                        ...
                        EmojiLastSelectedView.iv_border.setVisibility(GONE);
                        EmojiLastSelectedView.iv_scale.setVisibility(GONE);
                        EmojiLastSelectedView.iv_delete.setVisibility(GONE);
                        EmojiLastSelectedView.iv_flip.setVisibility(GONE);
                        if (LastSelectedView != null) {
                            LastSelectedView.iv_border.setVisibility(GONE);
                            LastSelectedView.iv_scale.setVisibility(GONE);
                            LastSelectedView.iv_delete.setVisibility(GONE);
                            LastSelectedView.iv_flip.setVisibility(GONE);
                        }

                        EmojiLastSelectedView = (EmojiStickerView) view;
...
//                            iv_flip.setVisibility(View.VISIBLE);
                    ...
                    case MotionEvent.ACTION_MOVE:
                        ...
                        EmojiStickerView.this.setX(EmojiStickerView.this.getX() + offsetX);
                        EmojiStickerView.this.setY(EmojiStickerView.this.getY() + offsetY);
                        move_orgX = event.getRawX();
                        move_orgY = event.getRawY();
                    ...
                    case MotionEvent.ACTION_UP:
                        ...
                        LastSelectedView.iv_delete.setVisibility(GONE);
                        LastSelectedView.iv_flip.setVisibility(GONE);
                        LastSelectedView =(StickerView) view;
                ...
                switch (event.getAction()) {
                    case MotionEvent.ACTION_DOWN:
                        ...
                        this_orgX = EmojiStickerView.this.getX();
                        this_orgY = EmojiStickerView.this.getY();

                        scale_orgX = event.getRawX();
                        scale_orgY = event.getRawY();
                        scale_orgWidth = EmojiStickerView.this.getLayoutParams().width;
                        scale_orgHeight = EmojiStickerView.this.getLayoutParams().height;

                        rotate_orgX = event.getRawX();
                        rotate_orgY = event.getRawY();

                        centerX = EmojiStickerView.this.getX() +
                                ((View) EmojiStickerView.this.getParent()).getX() +
                                (float) EmojiStickerView.this.getWidth() / 2;
                        ...
                        centerY = EmojiStickerView.this.getY() +
                                ((View) EmojiStickerView.this.getParent()).getY() +
                                statusBarHeight +
                                (float) EmojiStickerView.this.getHeight() / 2;
                    ...
                    case MotionEvent.ACTION_MOVE:
                        ...
                        if (length2 > length1
                            ...
                            EmojiStickerView.this.getLayoutParams().width += offset;
                            EmojiStickerView.this.getLayoutParams().height += offset;
                            onScaling(true);
                            //DraggableViewGroup.this.setX((float) (getX() - offset / 2));
                            //DraggableViewGroup.this.setY((float) (getY() - offset / 2));
                        } else if (length2 < length1
                                && (angle_diff < 25 || Math.abs(angle_diff - 180) < 25)
                                && EmojiStickerView.this.getLayoutParams().width > size / 2
                                && EmojiStickerView.this.getLayoutParams().height > size / 2) {
                            //scale down
                            double offsetX = Math.abs(event.getRawX() - scale_orgX);
                            double offsetY = Math.abs(event.getRawY() - scale_orgY);
                            double offset = Math.max(offsetX, offsetY);
